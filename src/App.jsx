...
function App() {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      const emissions = mockEmissionsData.filter(
        (e) => e.userId === userData.id
      );
  ...
  const addEmission = (emissionData) => {
    ...
    const totalEmissions = updatedEmissions.reduce(
      (sum, e) => sum + e.amount,
      0
    );
  ...
  const renderCurrentPage = () => {
    ...
    switch (currentPage) {
      case "data-entry":
        return (
          <DataEntry onAddEmission={addEmission} onNavigate={navigateTo} />
        );
      case "analytics":
      ...
      case "profile":
        return (
          <Profile user={user} setUser={setUser} emissions={userEmissions} />
        );
      default:
        return (
          <Dashboard
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      <main
        className={user ? "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6" : ""}
      >
        {renderCurrentPage()}
      </main>
    </div>
