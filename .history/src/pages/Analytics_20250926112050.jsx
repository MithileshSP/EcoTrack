...
const Analytics = ({ emissions }) => {
  ...
  const processedData = React.useMemo(() => {
    ...
    filteredEmissions.forEach(emission => {
      ...
      if (!monthlyData[month]) {
        monthlyData[month] = { month, total: 0, transportation: 0, energy: 0, food: 0, other: 0 };
      }
  ...
  const userVsAverages = [
    { name: 'Transportation', user: categoryTotals.transportation || 0, national: 1050, global: 1850 },
    { name: 'Energy', user: categoryTotals.energy || 0, national: 1150, global: 1950 },
    { name: 'Food', user: categoryTotals.food || 0, national: 450, global: 750 },
    { name: 'Other', user: categoryTotals.other || 0, national: 150, global: 250 }
  ];

  const totalEmissions = Object.values(categoryTotals).reduce((sum, val) => sum + val, 0);
  const percentageVsNational = ((totalEmissions - comparisonData.national) / comparisonData.national) * 100;
  const percentageVsGlobal = ((totalEmissions - comparisonData.global) / comparisonData.global) * 100;
  const targetProgress = totalEmissions === 0 ? 0 : (comparisonData.target / totalEmissions) * 100;
  ...
  const handleExport = React.useCallback(() => {
    ...
    const headers = ['Date', 'Category', 'Type', 'Amount (kg CO₂e)', 'Description'];
    ...
    link.href = url;
    link.download = `emissions-${timeRange}-${selectedCategory}-${new Date().toISOString().split('T')[0]}.csv`;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg p-6 shadow-sm border">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Detailed analysis of your carbon footprint</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">vs National Avg</p>
              <p className="text-2xl font-bold text-gray-900">
                {percentageVsNational > 0 ? '+' : ''}{percentageVsNational.toFixed(1)}%
              </p>
              <div className="flex items-center mt-1">
                ...
                <p className={`text-xs ${percentageVsNational < 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {Math.abs(percentageVsNational).toFixed(1)}% {percentageVsNational < 0 ? 'below' : 'above'} average
                </p>
              </div>
            </div>
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {percentageVsGlobal > 0 ? '+' : ''}{percentageVsGlobal.toFixed(1)}%
              </p>
              <div className="flex items-center mt-1">
                ...
                <p className={`text-xs ${percentageVsGlobal < 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {Math.abs(percentageVsGlobal).toFixed(1)}% {percentageVsGlobal < 0 ? 'below' : 'above'} average
                </p>
              </div>
            </div>
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {((comparisonData.target / totalEmissions) * 100).toFixed(0)}%
              </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border">
          <h3 className="text-lg font-medium text-gray-900 mb-4">Top Emission Sources</h3>
          <ResponsiveContainer width="100%" height={300}>
        ...
        {/* Comparison Chart */}
        <div className="bg-white rounded-lg p-6 shadow-sm border">
          <h3 className="text-lg font-medium text-gray-900 mb-4">You vs Averages</h3>
          <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="flex items-start p-4 bg-blue-50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700">
                  {Object.entries(categoryTotals).sort(([,a], [,b]) => b - a)[0]?.[0]?.charAt(0).toUpperCase() + Object.entries(categoryTotals).sort(([,a], [,b]) => b - a)[0]?.[0]?.slice(1)} accounts for {
                    ((Object.entries(categoryTotals).sort(([,a], [,b]) => b - a)[0]?.[1] || 0) / totalEmissions * 100).toFixed(0)
                  }% of your emissions
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start p-4 bg-green-50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-green-700">
                  You need to reduce emissions by {(totalEmissions - comparisonData.target).toFixed(1)} kg CO₂e to meet Paris Agreement targets
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start p-4 bg-yellow-50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700">
                  {monthlyData.length > 1 && monthlyData[monthlyData.length - 1].total > monthlyData[monthlyData.length - 2].total
        ...
        {/* Personal Metrics */}
        <div className="bg-white rounded-lg p-6 shadow-sm border">
          <h3 className="text-lg font-medium text-gray-900 mb-4">Personal Metrics</h3>
          <div className="space-y-6">
            <div>
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm font-medium text-gray-700">Daily Average</span>
                <span className="text-sm text-gray-900">{(totalEmissions / 30).toFixed(1)} kg CO₂e</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div 
                  className="bg-green-600 h-2 rounded-full" 
                  style={{width: `${Math.min((totalEmissions / 30) / 10 * 100, 100)}%`}}
                ></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm text-gray-900">{(totalEmissions / 4).toFixed(1)} kg CO₂e</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div 
                  className="bg-blue-600 h-2 rounded-full" 
                  style={{width: `${Math.min((totalEmissions / 4) / 70 * 100, 100)}%`}}
                ></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm text-gray-900">{(totalEmissions * 12).toFixed(0)} kg CO₂e</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div 
                  className="bg-red-600 h-2 rounded-full" 
                  style={{width: `${Math.min((totalEmissions * 12) / 5000 * 100, 100)}%`}}
                ></div>
              </div>
            </div>
            ...
            <div className="pt-4 border-t">
              <p className="text-sm text-gray-600">
                ...
                <span className="font-medium">{(totalEmissions * 12).toFixed(0)} kg CO₂e</span> annually.
              </p>
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {Object.entries(categoryTotals).map(([category, amount]) => {
                ...
                const nationalData = userVsAverages.find(d => d.name.toLowerCase() === category);
                const vsNational = nationalData ? ((amount - nationalData.national) / nationalData.national) * 100 : 0;
                const vsGlobal = nationalData ? ((amount - nationalData.global) / nationalData.global) * 100 : 0;
                ...
                return (
                  <tr key={category} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 text-right">{amount.toFixed(1)}</td>
                    <td className="py-3 px-4 text-right">{percentage.toFixed(1)}%</td>
                    <td className={`py-3 px-4 text-right ${vsNational < 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {vsNational > 0 ? '+' : ''}{vsNational.toFixed(1)}%
                    </td>
                    <td className={`py-3 px-4 text-right ${vsGlobal < 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {vsGlobal > 0 ? '+' : ''}{vsGlobal.toFixed(1)}%
                    </td>
                  </tr>
                );
              })}
